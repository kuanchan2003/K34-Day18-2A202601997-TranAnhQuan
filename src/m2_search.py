...
class BM25Search:
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        top_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:max(top_k, 0)]
        results = []
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        if not chunks:
            raise ValueError("Cannot build a dense index from an empty chunk list")
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    sorted_items = sorted(rrf_scores.items(), key=lambda kv: kv[1]["score"], reverse=True)
    ...
    return [
        ...
        for item in sorted_items[:max(top_k, 0)]
    ]
